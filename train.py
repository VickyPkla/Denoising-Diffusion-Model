...
class UNet(nn.Module):
    ...
    def forward(self, x, t):
        # Get time embeddings
        t_emb_16 = get_time_embedding(t, 16, 256)  # For enc1 and dec1
        t_emb_64 = get_time_embedding(t, 64, 128)  # For enc2 and dec2
        t_emb_128 = get_time_embedding(t, 128, 64)  # For enc3 and dec3
        t_emb_256 = get_time_embedding(t, 256, 32)  # For enc4 and dec4
...
def add_noise(hr_images, gamma_t):
    ...
    noisy_images = torch.sqrt(gamma_t) * hr_images + torch.sqrt(1 - gamma_t) * noise
    return noisy_images, noise

# Function to train DDPM model
def train_ddpm(model, dataloader, num_timesteps=1000, num_epochs=100, lr=1e-4):
    ...
    for epoch in range(num_epochs):
        for images in dataloader:
            ...
            t = torch.randint(0, num_timesteps, (images.shape[0],)).long().to(device)
            gamma_t = gamma[t].view(-1, 1, 1, 1)

            noisy_images, noise = add_noise(images, gamma_t)
...
# Denoising function
def denoise_image(model, input_image, num_timesteps):
...
if __name__ == "__main__":
    ...
    train_ddpm(model, dataloader, num_timesteps=1000, num_epochs=100, lr=2e-4)
